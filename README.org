...
*This is not useful if you already have emacs installed but I usually build it from sources to have the latest updates*
...
- Once
  ...
  and
  #+begin_src sh
    ./configure --with-native-compilation --with-modules --with-harfbuzz --without-compress-install --with-threads --with-included-regex --with-x-toolkit=gtk3 --with-zlib --with-jpeg --with-png --with-imagemagick --with-tiff --with-xpm --with-gnutls --with-xft --with-xml2 --with-mailutils --with-tree-sitter CFLAGS="-march=native -mtune=native -O2 -g3" --with-pgtk
  #+end_src
